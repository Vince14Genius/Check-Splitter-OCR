...
@Observable class SplitterFlowState {
    ...
    var canCalculate: Bool {
        ...
        items.allSatisfy { item in shares.contains { $0.has(item) } } &&
        payers.allSatisfy { payer in shares.contains { $0.has(payer) } }
    }
    
//    func sanitizeShares() {
//        shares.removeAll { share in
//            !(items.contains { share.has($0) } && payers.contains { share.has($0) })
//        }
//    }
}
